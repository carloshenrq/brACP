...
<div class="modal" ng-controller="account.password" ng-init="passwordInit({if $smarty.const.BRACP_ALLOW_ADMIN_CHANGE_PASSWORD}true{else}false{/if}, {$account->getGroup_id()}, {$smarty.const.BRACP_ALLOW_ADMIN_GMLEVEL})">
    ...
    <div class="modal-body" ng-if="stage == 0">
        ...
        <form class="modal-form" ng-submit="submitPassword()">
            ...
            <input type="text" ng-model="user_pass" placeholder="@@CHANGEPASS,HOLDER(ACTUAL_PASSWORD)" size="32" pattern="{$smarty.const.BRACP_REGEXP_PASSWORD}" required/>
            <input type="text" ng-model="user_pass_new" placeholder="@@CHANGEPASS,HOLDER(NEW_PASSWORD)" size="32" maxlength="32" pattern="{$smarty.const.BRACP_REGEXP_PASSWORD}" required/>
            <input type="text" ng-model="user_pass_conf" placeholder="@@CHANGEPASS,HOLDER(CONFIRM_PASSWORD)" size="32" maxlength="32" pattern="{$smarty.const.BRACP_REGEXP_PASSWORD}" required/>
